...
const tabs = ['All', 'Celebrations', 'Decor & Details', 'Venues'] as const;

export default function GalleryParties() {
  const [filter, setFilter] = useState<(typeof tabs)[number]>('All');
  const [lightbox, setLightbox] = useState<string | null>(null);

  const rawItems = useMemo(() => [
    ...images.parties.map(src => ({ src, cat: 'Celebrations' })),
    ...images.decor.map(src => ({ src, cat: 'Decor & Details' })),
      ...
      <section className="mx-auto max-w-[1400px] px-6 pb-12 pt-40 md:px-10">
        <Reveal>
          ...
          <h1 className="mt-6 font-display text-5xl font-light text-pearl md:text-8xl">
            Party <span className="italic text-blush">Gallery</span>
          </h1>
          <p className="mt-4 max-w-xl text-sm leading-relaxed text-pearl/65">
            Explore our curated celebrations — high-energy private parties, milestone birthdays,
            atmospheres designed with light, sound, and pure entertainment.
          </p>
        </Reveal>
        ...
        <div className="relative mx-auto max-w-3xl px-6 py-28 text-center">
          <Reveal>
            <h2 className="font-display text-4xl leading-tight text-pearl md:text-5xl">
              Nights worth remembering forever.
            </h2>
            <div className="mt-8 flex justify-center gap-4">
              ...
              <MagneticButton to="/gallery/weddings" className="border border-champagne/45 text-champagne hover:bg-champagne hover:text-ink">
                See Wedding Gallery
              </MagneticButton>
            </div>
          </Reveal>
      ...
      <AnimatePresence>
        {lightbox && (
          ...
          >
            ...
            <motion.img
              src={lightbox}
              alt="Lunaire party event, enlarged"
